...
#pragma omp parallel for
	for (int i = 0; i < NUM_TRAIN; i++)	{
		...
		for (int k = 0; k < NUM_GT_BOX; k++){
			...
			int nS = gtBndBoxSampling(bbgt, bbs, bbR);//有效正样本数
			for (int j = 0; j < nS; j++){
				bbs[j][2] = min(bbs[j][2], im3u.cols);
				bbs[j][3] = min(bbs[j][3], im3u.rows);
				Mat mag1f = getFeature(im3u, bbs[j]), magF1f;//提取特征
				flip(mag1f, magF1f, CV_FLIP_HORIZONTAL);//水平翻转
				xP.push_back(mag1f);
				xP.push_back(magF1f);
				szP.push_back(bbR[j]);
				szP.push_back(bbR[j]);
...
{
	//setColorSpace(HSV);
	trainObjectness(numDetPerSize);//训练模型
	loadTrainedModel();//加载模型
	//illustrate();
...
bool Objectness::computeSaliencyImpl(InputArray image, OutputArray objectnessBoundingBox)
{
	...
	for (unsigned int i = 0; i < valIdxesSize; i++)
...
{
	CStr resDir = _voc.localDir + "ResIlu0/";
	CmFile::MkDir(resDir);
	const int TEST_NUM = _voc.testSet.size();
	for (int i = 0; i < TEST_NUM; i++){
		const vector<Vec4i> &boxesGT = _voc.gtTestBoxes[i];
